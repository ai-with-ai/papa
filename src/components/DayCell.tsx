...
export function DayCell({ day, fecha, isToday, turnos, notas, onToggle, onSetHora, onSaveNota }: Props) {
  ...
  return (
    <div
      ref={ref}
      className={`relative rounded-lg border p-2 min-h-[120px] min-w-[140px] transition-colors ${
        isToday
                      ...
                      personas.map(p => {
                        ...
                        return (
                          <div key={p} className="flex items-center gap-1 w-full">
                            <span className={`text-xs leading-none px-1.5 py-0.5 rounded-full font-medium ${PERSONA_COLOR[p]}`}>
                              {PERSONA_INITIAL[p]}
                            </span>
                            {custom && (
                              <span className="ml-auto text-xs font-medium bg-white text-black px-1.5 py-0.5 rounded-full leading-none">
                                {hora}h
                              </span>
                            )}
                          </div>
